...
export const SystemMonitor: React.FC = () => {
  ...
  // Kalkulasi status kritis (untuk styling dinamis)
  const isCritical = useMemo(() => stats.ramUsedPercent > 80, [stats.ramUsedPercent]);

  useEffect(() => {
    const fetchStats = async () => {
      try {
        if ((window as any).api?.getGpuUsage) {
          const res = await (window as any).api.getGpuUsage();
          if (res) {
            setStats(res);
            
            // Kalkulasi % VRAM (Handle division by zero)
            ...
            setData(prev => {
              ...
              return newData.slice(-15); // History lebih panjang (15 titik)
            });
          }
        }
      } catch (err) {
  ...
  return (
    <div style={{ marginBottom: '24px' }}>
      <Row gutter={[16, 16]}>
        ...
        {/* WIDGET KIRI: ANALYTICS CHART */}
        <Col span={16}>
          <Card 
            bordered={false} 
            style={{...localStyles.mainCard, borderTop: `3px solid ${isCritical ? '#f5222d' : dmsTheme.colors.primary}`}}
            bodyStyle={{ padding: '20px' }}
          >
            <div style={localStyles.cardHeader}>
              <Space size={12}>
                <div style={{...localStyles.iconContainer, background: isCritical ? '#fff1f0' : dmsTheme.colors.secondary}}>
                ...
                <div>
                  ...
                  <Space split={<DividerVertical />}>
                    <Text type="secondary" style={localStyles.subText}>LIVE_UTILIZATION</Text>
                    <Text style={{...localStyles.subText, color: dmsTheme.colors.status.success}}>STABLE_LINK</Text>
                  </Space>
                </div>
              </Space>
        ...
        {/* WIDGET KANAN: HARDWARE STATUS */}
        <Col span={8}>
          <Card bordered={false} style={localStyles.mainCard} bodyStyle={{ padding: '20px' }}>
            <Space direction="vertical" size={20} style={{ width: '100%' }}>
              
              {/* RAM MONITOR */}
              <div style={localStyles.statRow}>
                <div style={localStyles.flexBetween}>
                  <Text strong style={localStyles.sectionLabel}>MEM_POOL_USAGE</Text>
                  <AntTooltip title="Physical RAM allocated by OS">
                    <AiOutlineInfoCircle size={12} color="#94a3b8" />
                  </AntTooltip>
                </div>
                <div style={localStyles.valueBox}>
              ...
              {/* GPU ENGINE - PRO STYLE */}
              <motion.div 
                animate={{ boxShadow: isCritical ? "0 0 15px rgba(245,34,45,0.2)" : "0 0 0px rgba(0,0,0,0)" }}
                style={localStyles.gpuEngineBox}
              >
                <div style={localStyles.flexBetween}>
                  <Space>
                    <AiOutlineThunderbolt color={dmsTheme.colors.accent} className="glow-icon" />
                    <Text style={localStyles.gpuLabel}>GPU_ACCELERATION</Text>
                  </Space>
                  <Badge color={dmsTheme.colors.accent} />
                </div>
                <div style={localStyles.gpuInfoInner}>
                  <Text ellipsis style={localStyles.gpuModelText}>{stats.gpuModel}</Text>
                  <Row gutter={8} style={{marginTop: '10px'}}>
                    <Col span={12}>
                      <Text style={localStyles.microLabel}>VRAM_TOTAL</Text>
                      <Text style={localStyles.microValue}>{stats.vramTotal}MB</Text>
                    </Col>
                    <Col span={12}>
                      <Text style={localStyles.microLabel}>BUS_LOAD</Text>
                      <Text style={localStyles.microValue}>{stats.vramUsed > 0 ? 'ACTIVE' : 'IDLE'}</Text>
                    </Col>
                  </Row>
                </div>
              </motion.div>
...
const DividerVertical = () => <span style={{ color: '#e2e8f0' }}>|</span>;

const CustomTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    return (
      <div style={localStyles.customTooltip}>
