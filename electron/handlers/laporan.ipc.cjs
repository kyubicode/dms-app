...
function registerLaporanHandlers() {
  ...
  ipcMain.handle('dashboard:get-stats', async () => {
    try {
      ...
      const totalDokumentasi = db.prepare('SELECT COUNT(*) as count FROM dokumentasi').get().count;
      const totalFoto = db.prepare('SELECT COUNT(*) as count FROM table_foto').get().count;
      const recentLogs = db.prepare('SELECT * FROM logs ORDER BY timestamp DESC LIMIT 5').all();
      return { totalLaporan, totalDokumentasi, totalFoto, recentLogs };
    } catch (error) { return { totalLaporan: 0, totalDokumentasi: 0, totalFoto: 0, recentLogs: [] }; }
  });
  ...
  ipcMain.handle('laporan:update', async (event, data) => {
    const transaction = db.transaction(() => {
      ...
      if (oldData && oldData.nama_laporan !== data.nama_laporan) {
        ...
        // 2. Sinkronisasi Folder Fisik
        // Cek apakah folder lama ada, jika ada baru di-rename
        if (fs.existsSync(oldPath)) {
          try {
            fs.renameSync(oldPath, newPath);
            console.log("Folder fisik berhasil di-rename.");
          } catch (e) {
            throw new Error("Folder sedang dibuka aplikasi lain (File Explorer/Photo Viewer).");
          }
        } else if (fs.existsSync(newPath)) {
          // Jika folder lama GAK ADA tapi folder baru ADA (kasus Anda sekarang)
          // Kita tidak perlu error, lanjut ke update database saja
          console.log("Folder fisik sudah sesuai nama baru, lanjut update database.");
        }

        // 3. UPDATE PATH DI DATABASE (WAJIB JALAN)
        // Gunakan nama tabel 'dokumentasi' bukan 'table_dokumentasi'
        ...
        const result = updatePathStmt.run(oldPath, newPath, data.id_laporan);
        console.log(`Berhasil mengupdate ${result.changes} path foto di database.`);
      }
...
// --- TAMBAHKAN INI DI MAIN PROCESS ---
  ipcMain.handle('laporan:addFotoToDokumentasi', async (event, { id_laporan, id_dokumentasi, files }) => {
    try {
      // 1. Ambil info laporan dan album untuk menentukan path folder
      ...
      // 3. Proses copy file satu per satu
      for (const file of files) {
        const fileName = `${Date.now()}_${path.basename(file.path)}`;
        const destPath = path.join(targetDir, fileName);
  ...
  ipcMain.handle('laporan:renameDokumentasi', async (event, { id_dokumentasi, newName }) => {
      try {
          ...
          if (!newName || newName.trim() === "") {
              throw new Error("Nama dokumentasi baru tidak boleh kosong");
          }

          // 2. Ambil data lama untuk cari folder fisiknya
          ...
          if (!album) {
              return { success: false, message: "Album tidak ditemukan di database" };
          }

          const oldFolderName = sanitize(album.nama_dokumentasi);
          const newFolderName = sanitize(newName);
          const projectPath = path.join(uploaderPath, sanitize(album.nama_laporan));
          
          const oldPath = path.join(projectPath, oldFolderName);
          const newPath = path.join(projectPath, newFolderName);

          // 3. Rename Folder Fisik jika ada
          if (fs.existsSync(oldPath)) {
              // Cek jika folder tujuan sudah ada (biar nggak bentrok)
          ...
          // 4. Update Database (Update path foto yang ada di dalam album ini)
          ...
          db.transaction(() => {
              for (const f of fotos) {
                  // Ganti bagian path folder lama ke baru
                  const updatedFilePath = f.path_foto.replace(oldPath, newPath);
                  updateFotoStmt.run(updatedFilePath, f.id_foto);
              }

              // 5. Update Nama Album Utama
              db.prepare('UPDATE dokumentasi SET nama_dokumentasi = ? WHERE id_dokumentasi = ?')
                .run(newName, id_dokumentasi);
          })();

          return { success: true, message: "Album berhasil di-rename" };

      } catch (error) {
          console.error("IPC_RENAME_ERROR:", error);
